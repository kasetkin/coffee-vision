...
| **A4** *(configured 2026-09-03; NOT yet complete)* | DVC remote is registered and working: `remoteconfig` at `ssh://coffdvc@dvcremote:49081/...`, and `dvc push` succeeds from a shell holding the key. **But a plain `dvc push` only sends what the current workspace references.** Demonstrated: exp163's checkpoint `a44450ba` sits in the local cache and is absent from the current `dvc.lock`, so the push cannot see it. Finish with **`dvc push --all-commits`** (`-A`) | §3i: the point of A4 was the ~4.7 GB of archived fold checkpoints reachable only through old `dvc.lock` revisions. Those are exactly the objects a workspace-scoped push skips, so the item looks done while protecting almost none of what it was for | ~14 GB to transfer, one command |
