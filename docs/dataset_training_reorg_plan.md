...
| **C1-next** | **Re-sweep the band on the 5 newer rigs — no counting session needed.** VLM counting was validated against all 30 human-counted crops (2026-09-03): scored against VLM counts instead of human ones, the sweep gives the **same winner, same ordering, every MAPE within 1pp and every correlation within 0.013**, because a paired same-crop comparison cancels per-crop counting error. Generate crops on iPhone/oneplus/08-30, count them, re-sweep, and check the monotone MAPE/correlation improvement replicates | The band result currently rests on 3 rigs from 2026-08-11. **Do not use rig-bias spread as the decision metric**: per-crop bias sd 0.055 gives SE 0.0175 at n=10/rig, and the human's own within-rig CV (4.3-7.6%) gives SE ~0.016 — neither counter resolves the 0.012 effect at that n, ~190 crops/rig would be needed | ~2-3h, no human labour |
