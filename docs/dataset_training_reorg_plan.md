...
| **C1-ship** | **Validate `_K_LO=5` (+ its refitted `CALIBRATION_K`) through a `run_folds` cross-rig sweep** before changing either constant | The band result is offline *reconstruction* accuracy, not classification accuracy. Per this project's own rule a config change is validated on folds, never on an offline benchmark. Both constants must move together — they are not separable | ~1-2 days compute |
